...
const SalaryPage = () => {
    ...
    const handleSearch = () => {
    // If no searchText, reset to original salary list
    if (!searchText.trim()) {
        setSalary(originalSalaryList);  // originalSalaryList should store unfiltered data
        return;
    }
            ...
            <div className='grid lg:grid-cols-3 sm:grid-cols-2 grid-cols-1 gap-12 my-12 items-center'>
                {
                    salary.map((data) => (
                        <div key={data.id} className='shadow px-4 py-8'>
                            <h4 className='font font-semibold text-xl '>{data.title}</h4>
